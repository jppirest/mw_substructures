...
def global_var(x):
    ...
    isocronas = pd.read_csv('../../../Isocronas_Gaia/gaia_metal.csv')
    isocronas = isocronas[isocronas.label <= 4]
    isocronas = isocronas[isocronas.MH == -0.1]
    isocronas['BP-RP'] = isocronas['G_BPmag'] - isocronas['G_RPmag']
...
def chi_to_age(IDADE):
  ...
  for k in range(len(modulo_distancia)):
    Yiso[k] = data['Gmag'] + modulo_distancia[k] + Ag
...
def ajuste_inicial(show = False, show_final = False, save = False):
    ...
    arrays_de_incremento = np.arange(0,0.55,passo)
...
def final(show = False, save = True):
    ...
    f = open('output_doitall/valores_chi.csv', 'w')
    f.write('chi,idade,dist\n')
    for element in np.sort(resultado_chi.flatten()):
        locals = np.where(resultado_chi==element)
        idadechi_ = newage[locals[0][0]]
        distchi_ = modulo_distancia[locals[1][0]]
        f.write(str(element) + ',' + str(idadechi_) + ',' + str(distchi_) + '\n')
    f.close()
    ...
    if save == True:
        ...
        im  = ax.contourf(x, y, resultado_chi, levels = 300, antialiased=False, cmap=cmap)#, norm=matplotlib.colors.LogNorm())
        cbar = fig.colorbar(im)
        cbar.set_label(r'$ \mathbf{\chi^2}$', fontweight = 'bold', rotation=0, labelpad=15)
        ax.set_xlabel(r'$\mathbf{(m - M)_o}$', fontweight = 'bold', labelpad=10)
        ax.set_ylabel('log(Age)', fontweight = 'bold', labelpad=10)
